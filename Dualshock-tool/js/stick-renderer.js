...
export function draw_stick_dial(ctx, center_x, center_y, sz, stick_x, stick_y, opts = {}) {
    const { circularity_data = null, enable_zoom_center = false, highlight, deadzone_radius = 0 } = opts;
    ...
    // Draw deadzone visualization if provided
    if (deadzone_radius && deadzone_radius > 0) {
        const dzr = Math.max(0, Math.min(0.49, deadzone_radius));
        ctx.fillStyle = 'rgba(41, 137, 247, 0.10)'; // light blue fill
        ctx.strokeStyle = 'rgba(41, 137, 247, 0.6)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.arc(center_x, center_y, sz * dzr, 0, 2 * Math.PI);
        ctx.fill();
        ctx.stroke();
    }
