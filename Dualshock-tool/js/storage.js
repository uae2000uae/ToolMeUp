...
export const Storage = {
  STORAGE_KEYS: {
    ...
    STICK_DEADZONE_PERCENT: 'stickDeadzonePercent',
    STICK_DEADZONE_BY_SERIAL: 'stickDeadzoneBySerial',
    DEADZONE_MODE: 'deadzoneMode',
    FINETUNE_BACKUPS: 'finetuneBackups',
  },
  ...
  finetuneBackups: {
    getAll() {
      return Storage.getObject(Storage.STORAGE_KEYS.FINETUNE_BACKUPS) || {};
    },
    setAll(backups) {
      Storage.setObject(Storage.STORAGE_KEYS.FINETUNE_BACKUPS, backups);
    },
    get(serialNumber) {
      if (!serialNumber) return null;
      const all = this.getAll();
      return Array.isArray(all[serialNumber]) ? all[serialNumber] : null;
    },
    set(serialNumber, finetuneArray) {
      if (!serialNumber || !Array.isArray(finetuneArray)) return;
      const all = this.getAll();
      all[serialNumber] = finetuneArray.slice();
      this.setAll(all);
    },
    clear(serialNumber) {
  ...
  deadzoneBySerial: {
    getAll() {
      return Storage.getObject(Storage.STORAGE_KEYS.STICK_DEADZONE_BY_SERIAL) || {};
    },
    setAll(map) {
      Storage.setObject(Storage.STORAGE_KEYS.STICK_DEADZONE_BY_SERIAL, map);
    },
    get(serialNumber) {
      if (!serialNumber) return null;
      const map = this.getAll();
      const val = map[serialNumber];
      return (typeof val === 'number') ? val : (val != null ? Number(val) : null);
    },
    set(serialNumber, percent) {
      if (!serialNumber) return;
      const p = Math.max(0, Math.min(40, Number(percent) || 0));
      const map = this.getAll();
      map[serialNumber] = p;
      this.setAll(map);
    },
