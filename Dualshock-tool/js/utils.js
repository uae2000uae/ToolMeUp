...
/**
* Apply a radial deadzone to joystick coordinates.
* - If magnitude <= deadzone, returns {0,0}.
* - Else, preserves direction and rescales magnitude so that (deadzone..1) maps to (0..1).
* @param {number} x - x in range [-1,1]
* @param {number} y - y in range [-1,1]
* @param {number} deadzone - threshold in [0, 0.49]
* @returns {{x:number,y:number}}
*/
export function applyRadialDeadzone(x, y, deadzone = 0) {
  const dz = Math.max(0, Math.min(0.49, Number.isFinite(deadzone) ? deadzone : 0));
  const r = Math.hypot(x, y);
  if (r <= dz) return { x: 0, y: 0 };
  // Normalize and rescale remaining range to [0,1]
