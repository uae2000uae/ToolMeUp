...
import { sleep, float_to_str, dec2hex, dec2hex32, lerp_color, initAnalyticsApi, la, applyRadialDeadzone } from './utils.js';
...
import { show_deadzone_modal, isDeadzoneVisible, deadzone_handle_controller_input } from './modals/deadzone-modal.js';
import { 
...
function gboot() {
  ...
  async function initializeApp() {
    ...
    // Initialize upload logger integration into the Firmware Update tab
    (function initUploadLogger(){
      if (window.__uploadLogger) return;

      const list = document.getElementById('fw-log-list');
      const clearBtn = document.getElementById('fw-log-clear');
      const checkpointBtn = document.getElementById('fw-log-checkpoint');

      const maxItems = 500;
      const prevRecvById = new Map();
      let checkpoint = null;
      ...
      function pushLine(text){
        ...
        while(list.childElementCount > maxItems) list.removeChild(list.firstChild);
      ...
      function envStr(env){
        try {
          const parts = [];
          if (env?.transport) parts.push(env.transport);
          if (env?.host) parts.push(env.host);
          if (env?.platform) parts.push(env.platform);
          return parts.join(' | ');
        } catch(_) { return ''; }
      }

      window.__uploadLogger = {
        logFeature({model, reportId, hex, tStart, tEnd, durationMs, env}){
          try {
            const base = `[${now()}] FEAT id=0x${Number(reportId).toString(16)} ${model||''}`;
            const extra = durationMs!=null ? ` ${Number(durationMs).toFixed(1)}ms` : '';
            const envtxt = env ? ` ${envStr(env)}` : '';
            pushLine(`${base}${extra}${envtxt} len=${hex?.length/2|0} ${hex}`);
          } catch(_){ }
        },
        logReceive({model, reportId, hex, tStart, tEnd, durationMs, env}){
          try {
            const base = `[${now()}] RECV id=0x${Number(reportId).toString(16)} ${model||''}`;
            const extra = durationMs!=null ? ` ${Number(durationMs).toFixed(1)}ms` : '';
            const envtxt = env ? ` ${envStr(env)}` : '';
            let suffix = '';
            if (checkpoint) {
              const before = prevRecvById.get(reportId);
              if (before && before !== hex) suffix = '  [DIFF]';
            }
            pushLine(`${base}${extra}${envtxt} len=${hex?.length/2|0} ${hex}${suffix}`);
            prevRecvById.set(reportId, hex);
          } catch(_){ }
        },
        logOutput({model, reason, hex}){
          try { pushLine(`[${now()}] OUT  ${reason?`(${reason}) `:''}${model||''} len=${hex?.length/2|0} ${hex}`); } catch(_){}
        },
        logInfo(msg){
          try { pushLine(`[${now()}] INFO ${msg}`); } catch(_){}
        },
        markCheckpoint(label = 'mark'){
          checkpoint = { t: Date.now(), label };
          pushLine(`[${now()}] ==== CHECKPOINT: ${label} ====`);
        },
        clear(){ if (list) list.innerHTML=''; prevRecvById.clear(); checkpoint = null; }
      };

      if (clearBtn) clearBtn.onclick = ()=>{ window.__uploadLogger.clear(); };
      if (checkpointBtn) checkpointBtn.onclick = ()=>{ window.__uploadLogger.markCheckpoint('user'); };

      // Environment snapshot function for instrumentation
      if (typeof window.__envSnapshot !== 'function') {
        window.__envSnapshot = function(){
          try {
            const transport = window.__transport || 'unknown';
            const host = (typeof navigator !== 'undefined') ? (navigator.userAgent.includes('PlayStation') ? 'PS5' : 'PC') : 'unknown';
            const platform = navigator?.platform || '';
            const focused = document?.visibilityState === 'visible';
            const url = location?.href || '';
            return { transport, host, platform, focused, url };
          } catch(_) { return {}; }
        };
      }
    })();
...
async function continue_connection({data, device}) {
  try {
    ...
    // Share current serial with controller manager for per-device settings
    try {
      controller.setCurrentSerialNumber(lastConnectedInfo.serialNumber);
    } catch (_) {}
...
window.refresh_stick_pos = function refresh_stick_pos() {
  ...
  // Values in controller.button_states.sticks are already processed by the unified input pipeline
  // (including DS5/Edge deadzone and rescaling when enabled). Use them directly here to avoid
  // applying the deadzone twice in the UI layer.
  const flx = plx, fly = ply;
  const frx = prx, fry = pry;

  const enable_zoom_center = center_zoom_checked();
  const enable_circ_test = circ_checked();

  // Draw left stick (apply deadzone to visuals)
  draw_stick_dial(ctx, hb, yb, sz, flx, fly, {
    circularity_data: enable_circ_test ? ll_data : null,
    enable_zoom_center,
  });

  if(!hasSingleStick) {
    // Draw right stick (apply deadzone to visuals)
    draw_stick_dial(ctx, w-hb, yb, sz, frx, fry, {
  ...
  $("#lx-lbl").text(float_to_str(flx, precision));
  $("#ly-lbl").text(float_to_str(fly, precision));
  if(!hasSingleStick) {
    $("#rx-lbl").text(float_to_str(frx, precision));
    $("#ry-lbl").text(float_to_str(fry, precision));
  }
...
function handleControllerInput({ changes, inputConfig, touchPoints, batteryStatus }) {
  ...
  // Handle Deadzone Modal input (can be open from any tab)
  if (isDeadzoneVisible()) {
    if (changes.sticks) deadzone_handle_controller_input(changes);
    return;
  }
...
// Deadzone mode management (simple/expert)
(function initDeadzoneMode(){
  try {
    app.deadzoneMode = Storage.getString(Storage.STORAGE_KEYS.DEADZONE_MODE) || 'simple';
  } catch(_) { app.deadzoneMode = 'simple'; }
  updateDeadzoneModeUI();
})();

function updateDeadzoneModeUI(){
  const simple = app.deadzoneMode === 'simple';
  const chkSimple = document.getElementById('check-dz-simple');
  const chkExpert = document.getElementById('check-dz-expert');
  if (chkSimple) chkSimple.style.display = simple ? '' : 'none';
  if (chkExpert) chkExpert.style.display = simple ? 'none' : '';
}

window.setDeadzoneMode = (mode, event) => {
  if (event) { event.preventDefault(); event.stopPropagation(); }
  app.deadzoneMode = (mode === 'expert') ? 'expert' : 'simple';
  Storage.setString(Storage.STORAGE_KEYS.DEADZONE_MODE, app.deadzoneMode);
  updateDeadzoneModeUI();
...
window.executeSelectedDeadzone = () => {
  const expert = app.deadzoneMode === 'expert';
  show_deadzone_modal(controller, expert);
};
