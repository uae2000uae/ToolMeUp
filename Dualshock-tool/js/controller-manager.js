...
class ControllerManager {
  constructor(uiDependencies = {}) {
    ...
    this.currentSerialNumber = null; // Cached serial for per-device settings
  ...
  */
  ...
  setCurrentSerialNumber(serial) {
    this.currentSerialNumber = serial || null;
  }
  ...
  */
  ...
  /**
   * Apply deadzone to device firmware (experimental) for DS5/DS5 Edge
   * @param {number} percent - 0..40
   */
  async applyDeadzoneToDevice(percent) {
    if (!this.currentController) {
      throw new Error(l("No controller connected"));
    }
    const model = this.getModel();
    if (model !== 'DS5' && model !== 'DS5_Edge') {
      throw new Error(l("Device-level deadzone is only supported on DualSense"));
    }
    if (typeof this.currentController.applyDeviceDeadzone !== 'function') {
      throw new Error(l("Controller does not support device-level deadzone write"));
    }
    try { window.__uploadLogger?.logInfo?.(`applyDeadzoneToDevice invoked: p=${Number(percent)||0}% for ${model}`); } catch(_){ }
    const res = await this.currentController.applyDeviceDeadzone(percent);
    // Mark changes pending as this is a modification request
    this.setHasChangesToWrite(true);
    return res;
  }

  /** Plan B (Expert): Apply deadzone by writing Module 12 finetune */
  async applyDeadzoneToDeviceFinetune(percent, { dryRun = false } = {}) {
    if (!this.currentController) {
      throw new Error(l("No controller connected"));
    }
    const model = this.getModel();
    if (model !== 'DS5' && model !== 'DS5_Edge') {
      throw new Error(l("Finetune deadzone is only supported on DualSense"));
    }
    if (typeof this.currentController.applyDeviceDeadzoneFinetune !== 'function') {
      throw new Error(l("Controller does not support finetune deadzone write"));
    }
    try { window.__uploadLogger?.logInfo?.(`applyDeadzoneToDeviceFinetune invoked: p=${Number(percent)||0}% for ${model}${dryRun?' (dryRun)':''}`); } catch(_){ }
    const res = await this.currentController.applyDeviceDeadzoneFinetune(percent, { dryRun });
    this.setHasChangesToWrite(true);
    return res;
  }
  ...
  async restoreFinetuneBackup() {
    ...
    const model = this.getModel();
    if (model !== 'DS5' && model !== 'DS5_Edge') {
      throw new Error(l("Restore is only supported on DualSense"));
    }
    if (typeof this.currentController.restoreFinetuneBackup !== 'function') {
      throw new Error(l("Controller does not support finetune restore"));
    }
    try { window.__uploadLogger?.logInfo?.(`restoreFinetuneBackup invoked for ${model}`); } catch(_){ }
    const res = await this.currentController.restoreFinetuneBackup();
    this.setHasChangesToWrite(true);
    return res;
  }
  ...
  */
  _recordButtonStates(data, BUTTON_MAP, dpad_byte, l2_analog_byte, r2_analog_byte) {
    ...
    let [new_lx, new_ly, new_rx, new_ry] = [0, 1, 2, 3]
      .map(i => data.getUint8(i))
      .map(v => Math.round((v - 127.5) / 128 * 100) / 100);

    // For DS5/DS5 Edge devices: apply a center-based per-axis deadzone in the
    // firmware processing path (our unified input pipeline) and rescale the remaining travel.
    // This keeps the center exact (neutral) and preserves full travel beyond the deadzone.
    try {
      const model = this.currentController?.getModel?.();
      if (model === 'DS5' || model === 'DS5_Edge' || model === 'DS4') {
        // Prefer per-device deadzone if available, fallback to global setting
        let dzPercent = null;
        try {
          dzPercent = Storage.deadzoneBySerial.get(this.currentSerialNumber);
        } catch (_) { dzPercent = null; }
        if (dzPercent === null || Number.isNaN(dzPercent)) {
          dzPercent = Number(Storage.getNumber(Storage.STORAGE_KEYS.STICK_DEADZONE_PERCENT, 10)) || 0;
        }
        dzPercent = Math.max(0, Math.min(40, dzPercent));
        const D = dzPercent / 100.0; // 0.0..0.4
        const applyAxisDeadzone = (v, D) => {
          const a = Math.max(-1, Math.min(1, Number(v)));
          const ad = Math.abs(a);
          if (ad <= D) return 0;
          // With normalized inputs around calibrated center, half-range H on each side is 1.
          // Rescale so that (D..1) maps to (0..1): sign(v) * (|v|-D)/(1-D)
          const denom = (1 - D) || 1; // safety
          return Math.sign(a) * ((ad - D) / denom);
        };
        new_lx = applyAxisDeadzone(new_lx, D);
        new_ly = applyAxisDeadzone(new_ly, D);
        new_rx = applyAxisDeadzone(new_rx, D);
        new_ry = applyAxisDeadzone(new_ry, D);
      }
    } catch (e) {
      // Fail-safe: if anything goes wrong, fall back to raw normalized values
      console.warn('Deadzone processing failed, using raw values:', e);
    }
